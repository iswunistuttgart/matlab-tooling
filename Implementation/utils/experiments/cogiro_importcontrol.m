...
assert(strcmpi('.lirmm', chExtension), 'Invalid file extension [%s] found. Must be [.mat].', chExtension);
...
vExp_Time = 0:1/dSamplingTime:(numel(stLoadedData.MainControl_Command_StartNewMove) - 1);
% vExp_Time = ascolumn(stLoadedData.MainControl_Command_StartNewMove(1,:));
% vExp_Command = ascolumn(stLoadedData.MainControl_Command_StartNewMove(2,:));
% vIndex_Command = vExp_Command == 1;
% vExp_Command = vExp_Command(vIndex_Command);
% vExp_Time = vExp_Time(vIndex_Command) - vExp_Time(find(vIndex_Command, 1));
...
if isfield(stLoadedData, 'Xcurrent_0_')
    aSetPose_Pos(:,1) = ascolumn(stLoadedData.Xcurrent_0_(2,vIndex_Command));
end
% Y position
if isfield(stLoadedData, 'Xcurrent_1_')
    aSetPose_Pos(:,2) = ascolumn(stLoadedData.Xcurrent_1_(2,vIndex_Command));
end
% Z position
if isfield(stLoadedData, 'Xcurrent_2_')
    aSetPose_Pos(:,3) = ascolumn(stLoadedData.Xcurrent_2_(2,vIndex_Command));
end
% A position
if isfield(stLoadedData, 'Xcurrent_3_')
    aSetPose_Pos(:,4) = ascolumn(stLoadedData.Xcurrent_3_(2,vIndex_Command));
end
% B position
if isfield(stLoadedData, 'Xcurrent_4_')
    aSetPose_Pos(:,5) = ascolumn(stLoadedData.Xcurrent_4_(2,vIndex_Command));
end
% C position
if isfield(stLoadedData, 'Xcurrent_5_')
    aSetPose_Pos(:,6) = ascolumn(stLoadedData.Xcurrent_5_(2,vIndex_Command));
end

%%% Velocity data
% X velocity
if isfield(stLoadedData, 'Vcurrent_0_')
    aSetPose_Vel(:,1) = ascolumn(stLoadedData.Vcurrent_0_(2,vIndex_Command));
end
% Y velocity
if isfield(stLoadedData, 'Vcurrent_1_')
    aSetPose_Vel(:,2) = ascolumn(stLoadedData.Vcurrent_1_(2,vIndex_Command));
end
% Z velocity
if isfield(stLoadedData, 'Vcurrent_2_')
    aSetPose_Vel(:,3) = ascolumn(stLoadedData.Vcurrent_2_(2,vIndex_Command));
end
% A velocity
if isfield(stLoadedData, 'Vcurrent_3_')
    aSetPose_Vel(:,4) = ascolumn(stLoadedData.Vcurrent_3_(2,vIndex_Command));
end
% B velocity
if isfield(stLoadedData, 'Vcurrent_4_')
    aSetPose_Vel(:,5) = ascolumn(stLoadedData.Vcurrent_4_(2,vIndex_Command));
end
% C velocity
if isfield(stLoadedData, 'Vcurrent_5_')
    aSetPose_Vel(:,6) = ascolumn(stLoadedData.Vcurrent_5_(2,vIndex_Command));
end

%%% Acceleration data
% X acceleration
if isfield(stLoadedData, 'Acurrent_0_')
    aSetPose_Acc(:,1) = ascolumn(stLoadedData.Acurrent_0_(2,vIndex_Command));
end
% Y acceleration
if isfield(stLoadedData, 'Acurrent_1_')
    aSetPose_Acc(:,2) = ascolumn(stLoadedData.Acurrent_1_(2,vIndex_Command));
end
% Z acceleration
if isfield(stLoadedData, 'Acurrent_2_')
    aSetPose_Acc(:,3) = ascolumn(stLoadedData.Acurrent_2_(2,vIndex_Command));
end
% A acceleration
if isfield(stLoadedData, 'Acurrent_3_')
    aSetPose_Acc(:,4) = ascolumn(stLoadedData.Acurrent_3_(2,vIndex_Command));
end
% B acceleration
if isfield(stLoadedData, 'Acurrent_4_')
    aSetPose_Acc(:,5) = ascolumn(stLoadedData.Acurrent_4_(2,vIndex_Command));
end
% C acceleration
if isfield(stLoadedData, 'Acurrent_5_')
    aSetPose_Acc(:,6) = ascolumn(stLoadedData.Acurrent_5_(2,vIndex_Command));
end
...
Collection = tscollection({tsSetPose_Pos, tsSetPose_Vel, tsSetPose_Acc}, 'Name', 'ControlCommand');
