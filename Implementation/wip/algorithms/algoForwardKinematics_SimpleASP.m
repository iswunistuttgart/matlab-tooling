function [Pose, varargout] = algoForwardKinematics_StandardASP(CableLength, PulleyPosition, CableAttachment, SolverOptions)
% ALGOFORWARDKINEMATICS_STANDARDASP estimates the robot pose with standard
%   pulley kinematics
% 
%   POSE = ALGOFORWARDKINEMATICS_STANDARDASP(CABLELENGTH, PULLEYPOSITION,
...
inOptimizationTargetVectorFunction = @(vEstimatedPose) algoForwardKinematics_StandardASP_TargetFunction(vEstimatedPose, vCableLength, aPulleyPosition, aCableAttachment);
...
function [VectorValuedFunction, Jacobian] = algoForwardKinematics_StandardASP_TargetFunction(EstimatedPose, TargetCableLength, PulleyPositions, CableAttachments)
...
vLengths = zeros(nNumberOfCables, 1);
...
vEvaluatedFunction = zeros(nNumberOfCables, 1);
for iCable = 1:nNumberOfCables
    ...
    t9 = t1*t8;
    t10 = sin(vRotation(1));
    t19 = (vPosition(1)+t1*t2*aCableAttachments(1,iCable)+(t9*t10-t5*t6)*aCableAttachments(2,iCable)+(t5*t10+t9*t6)*aCableAttachments(3,iCable)-aPulleyPositions(1,iCable)) ^ 2;
    t23 = t5*t8;
    t32 = (vPosition(2)+t5*t2*aCableAttachments(1,iCable)+(t1*t6+t23*t10)*aCableAttachments(2,iCable)+(-t1*t10+t23*t6)*aCableAttachments(3,iCable)-aPulleyPositions(2,iCable)) ^ 2;
    t39 = (t2*t10*aCableAttachments(2,iCable)+t2*t6*aCableAttachments(3,iCable)-t8*aCableAttachments(1,iCable)-aPulleyPositions(3,iCable)+vPosition(3)) ^ 2;
    t40 = vTargetCableLength(iCable).^2;
    vEvaluatedFunction(iCable) = t19+t32+t39-t40;
    vEvaluatedFunction(iCable) = t19+t32+t39;
end
% vEstimatedPose, [vEvaluatedFunction, vLengths(:).^2]
vEvaluatedFunction = vEvaluatedFunction(:) - vTargetCableLength(:).^2;
