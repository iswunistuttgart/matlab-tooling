function [X, ndx] = strnatsortfiles(X, varargin)
...
%%% Syntax:
%  Y = strnatsortfiles(X)
%  Y = strnatsortfiles(X,xpr)
%  Y = strnatsortfiles(X,xpr,<options>)
% [Y,ndx] = strnatsortfiles(X,...)
%
% To sort all of the strings in a cell array use STRNATSORT (File Exchange 34464).
% To sort the rows of a cell array of strings use STRNATSORTROWS (File Exchange 47433).
%
% See also STRNATSORT STRNATSORTROWS SORT SORTROWS NUM2SIP NUM2BIP CELLSTR REGEXP DIR FILEPARTS FULLFILE FILESEP
%
%% File Dependency %%
%
% STRNATSORTFILES requires the function STRNATSORT (File Exchange 34464). The inputs
% <xpr> and <options> are passed directly to STRNATSORT: see STRNATSORT for case
...
% STRNATSORTFILES splits filepaths at each file separator character and sorts
...
%    'test.m'
%    'test_new.m'
% strnatsortfiles(B) % Shorter names before longer (dictionary sort):
%  ans =
%    'test.m'
%    'test-old.m'
%    'test_new.m'
...
%    'test10.m'
%    'test2.m'
% strnatsortfiles(C) % Correct numeric order, shorter names before longer:
%  ans =
%    'test.m'
%    'test1.m'
%    'test2.m'
...
%    'A2-old\test.m'
%    'A2\test.m'
% strnatsortfiles(D) % Shorter names before longer (dictionary sort):
%  ans =
%    'A1archive.zip'
%    'A1\test.m'
%    'A2\test.m'
