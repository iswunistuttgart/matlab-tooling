function [c, ceq] = nonlcon(vX, aAnchorPositionsIn0, dE, dA0, dRho, dG, dFmin, dFmax)

% Nonlinear inequality constraints are
% f_max
% f_min
dRowsCEq = size(aAnchorPositionsIn0, 2)*2;
dColsCEq = numel(vX);

% Nonlinear equality constraints are
% x_{end, i}
% z_{end, i}
dRowsC = size(aAnchorPositionsIn0, 2)*2;
dColsC = numel(vX);
...
for iCable = 1:size(aAnchorPositionsIn0, 2)
    dOffset = (iCable-1)*2;
    % Position x
    ceq(iCable + 0 + dOffset) = vForcesX(iCable)*vLength(iCable)/(dE*dA0) + ...
        abs(vForcesX(iCable))/(dRho*dG)*(asinh(vForcesZ(iCable)/vForcesX(iCable)) - asinh((vForcesZ(iCable) - dRho*dG*vLength(iCable))/vForcesX(iCable))) - aAnchorPositionsIn0(1, iCable);
    % Position z
