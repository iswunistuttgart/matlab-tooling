...
%     U_hom(iTime+1,:) = transpose(B*transpose(U_hom(iTime,:))) - U_hom(iTime-1,:) + b(t(iTime));
    ...
    U_hom(iTime+1,2:end-1) = (2/(2 + a*dt)).*(transpose(B*transpose(U_hom(iTime,2:end-1))) + 1/2*(a*dt - 2).*U_hom(iTime-1,2:end-1) + b(t(iTime)));
    U_hom(iTime+1,end) = U_right(t(iTime));
    
    U_inh(iTime+1,1) = U_left(t(iTime));
    U_inh(iTime+1,2:end-1) = (2/(2 + a*dt)).*transpose(B*transpose(U_inh(iTime,2:end-1))) + 1/2*(a*dt - 2).* U_inh(iTime-1,2:end-1) + b(t(iTime)) + h(t(iTime),U_inh(iTime,2:end-1));
